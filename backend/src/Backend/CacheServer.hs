...
nixCacheRoutes = route
  [ ("nix-cache-info", cacheInfoHandler)
  , ("nar", pathArg narHandler)
  , ("", pathArg narinfoHandler)
  ]
...
narHandler narfile = do

narinfoHandler :: MonadSnap m => Text -> m ()
narinfoHandler file = do
  if T.isSuffixOf ".narinfo" file
    then
